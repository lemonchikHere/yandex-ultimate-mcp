...
3. Показывает, что вставить в OAuth app:
   ...
   - type: `Для доступа к API или отладки`
...
5. Wizard спрашивает только про optional localhost Redirect URI:
   - если **да** — ловит `access_token` сам через localhost;
   - если **нет** — использует официальный ручной flow: ты копируешь URL вида `https://oauth.yandex.ru/verification_code#access_token=...`.
6. Если видишь `400 redirect_uri не совпадает`, значит в OAuth app нет выбранного Redirect URI: используй ручной режим или добавь точный localhost URI.
...
- `YC_FOLDER_ID` / `YC_OAUTH_TOKEN` — Yandex Cloud; если установлен `yc`, wizard сам подставляет `yc config list`;
...
Wizard сам открывает страницы, использует официальный ручной OAuth flow или optional localhost auto-capture, вытаскивает `access_token`, подхватывает `yc config list` и пишет `.env.local` без дублей. Подробности: [`docs/AUTH.md`](docs/AUTH.md).
...
3. Тип приложения: **Для доступа к API или отладки**.
4. Redirect URI для ручного режима: `https://oauth.yandex.ru/verification_code`; localhost URI из wizard добавляй только если нужен auto-capture.
